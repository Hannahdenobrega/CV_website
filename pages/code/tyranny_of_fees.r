...
g <-   
  df_p %>% 
    ...
    ggplot() + 
    geom_line(aes(date, Gross), color = "steelblue", size = 1.5, alpha = 0.95) + 
    ...
    #theme_bw() +  
    geom_label(data = Msg, aes(date, Lvl, label = Msg), color = "steelblue", alpha = 0.8, size = 4) + 
...
g <- Cum_Fee_Comparison(Jalshtr, Fee = 50*1e-4, Start = ymd(20020101), 
                               # Added purely for figure adjustment:
                               Gap = 3, Lvlset = 7,
                               mnthfwd = 18)

ggsave("plots/plot_tyranny_of_fees.png", width = 8, height = 6)
